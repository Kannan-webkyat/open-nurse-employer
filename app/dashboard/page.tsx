...
const DashboardPage = () => {
  ...
  return (
    <DashboardLayout>
      ...
      <div className="space-y-5 animate-in fade-in duration-500 max-w-[1800px] mx-auto p-4 font-sans text-neutral-800">
        
        {/* Header - Compact */}
        <div className="flex items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-black text-neutral-900 tracking-tight">Dashboard Overview</h1>
            <p className="text-neutral-500 font-bold text-xs mt-0.5">Project analytics and hiring pipeline.</p>
          </div>

          <div className="flex items-center gap-3">
             {/* Search Bar - Compact */}
             <div className="hidden md:flex items-center bg-white px-3 py-2 rounded-xl shadow-sm border border-neutral-100 w-56 h-10">
                <Search className="w-3.5 h-3.5 text-neutral-400 mr-2" />
                <input type="text" placeholder="Search..." className="bg-transparent border-0 outline-none text-xs font-bold w-full placeholder:text-neutral-300" />
             </div>
             ...
             <Link href="/jobs">
                <Button className="h-10 px-4 bg-violet-600 hover:bg-violet-700 text-white shadow-lg shadow-violet-600/20 border-0 font-bold text-xs rounded-xl">
             ...
             {statsCards.map((stat, index) => {
                 const colorVariants: any = {
                    ...
                    violet: {
                        ...
                        iconWrapper: 'bg-violet-50 text-violet-600 group-hover:bg-violet-500 group-hover:text-white',
                        subBadge: 'bg-violet-50 text-violet-700',
                        gradient: 'from-violet-50'
                    }
                 };

                const variant = colorVariants[stat.color] || colorVariants.violet;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
            ...
            <div className="lg:col-span-8 space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                ...
                                .map((job: any, i: number) => {
                                    ...
                                    return (
                                        <div key={i} className="group flex items-center justify-between p-3 rounded-2xl bg-white border border-neutral-100/80 shadow-sm hover:shadow-md hover:border-amber-200 transition-all duration-300 cursor-pointer">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm transition-all duration-300 shadow-sm ${
                 ...
                 <div className="md:col-span-2 bg-white rounded-3xl p-6 shadow-sm border border-neutral-50 hover:shadow-lg transition-shadow duration-300 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-200 fill-mode-backwards">
                    <div className="flex justify-between items-center mb-6">
                        ...
                        <Button variant="outline" size="sm" className="rounded-xl border-neutral-200 text-xs font-bold hover:bg-neutral-50 hover:text-neutral-900 active:scale-95 transition-all">
                            ...
                            {normalizedPoints.map((p: number[], i: number) => (
                                <g key={i} className="group/point hover:scale-150 transition-transform origin-center cursor-pointer">
                                    <circle 
                                        ...
                                        stroke="#7C3AED" 
                                        strokeWidth="3"
                                        className="transition-all duration-300 group-hover/point:stroke-violet-500 group-hover/point:r-6"
                                    />
                                    ...
                                    <text 
                                        ...
                                        y={chartHeight + 20} 
                                        textAnchor="middle" 
                                        className="text-[10px] fill-neutral-400 font-bold opacity-0 group-hover/point:opacity-100 group-hover/point:fill-violet-600 transition-all duration-300 pointer-events-none"
                                    >
                                        {data.traffic[i]?.day}
                                    </text>
                                </g>
                ...
                <div className="bg-white rounded-3xl p-5 shadow-sm border border-neutral-50 relative overflow-hidden group/card hover:shadow-lg transition-all duration-500 animate-in fade-in slide-in-from-bottom-8 duration-700 delay-500 fill-mode-backwards">
                     ...
                     <div className="absolute inset-0 pointer-events-none opacity-30">
                        <svg className="absolute -right-10 -top-10 text-violet-50 w-64 h-64 rotate-12 group-hover/card:rotate-6 transition-transform duration-1000 ease-in-out" viewBox="0 0 200 200" fill="currentColor">
                    ...
                    <div className="flex items-center justify-between mb-6 relative z-10">
                        ...
                        <Button variant="ghost" size="sm" className="text-violet-600 font-bold hover:bg-violet-50 rounded-xl transition-colors">
                            View All
                        </Button>
                    </div>
                    ...
                    <div className="space-y-3 relative z-10">
                        {data.recent_jobs.slice(0, 4).map((job: any, i: number) => (
                            <div key={i} className="group flex items-center justify-between p-4 rounded-2xl bg-white border border-neutral-100 hover:border-violet-200 hover:shadow-md hover:shadow-violet-100 hover:-translate-x-1 transition-all duration-300 cursor-pointer">
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-xl bg-violet-50 text-violet-600 flex items-center justify-center font-black text-sm group-hover:scale-110 group-hover:bg-violet-600 group-hover:text-white transition-all duration-300 shadow-sm">
                                        {job.title.charAt(0)}
                                    </div>
                                    <div>
                                        <h4 className="font-bold text-neutral-900 group-hover:text-violet-700 transition-colors">{job.title}</h4>
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="flex -space-x-2">
                                        {[...Array(3)].map((_, idx) => (
                                            <div key={idx} className="w-6 h-6 rounded-full bg-neutral-100 border-2 border-white flex items-center justify-center text-[8px] font-bold text-neutral-400 group-hover:border-violet-100 transition-colors">
                                                U
                                            </div>
                                        ))}
                                    </div>
                                    <Badge variant={(job.status as any) || 'default'} className={`bg-transparent border-transparent ${
                                        job.status === 'active' ? 'text-emerald-600 bg-emerald-50' : 'text-neutral-500'
            ...
            <div className="lg:col-span-4 space-y-5 animate-in fade-in slide-in-from-right-8 duration-700 delay-500 fill-mode-backwards">
                 ...
                 {(() => {
                     ...
                     return (
                     <div className="bg-gradient-to-br from-violet-600 to-indigo-700 rounded-3xl p-5 text-white relative overflow-hidden shadow-lg shadow-violet-600/30 group hover:shadow-violet-600/50 hover:scale-[1.02] transition-all duration-500">
                        ...
                        <div className="relative z-10 flex flex-col gap-4">
                            <div className="flex justify-between items-start">
                                ...
                                <div className="bg-white/20 backdrop-blur-md p-1.5 rounded-lg shadow-sm group-hover:bg-white group-hover:text-violet-600 transition-colors duration-300"><CreditCard className="w-4 h-4" /></div>
                            </div>
                            ...
                            <Link href="/billing">
                                <Button size="sm" className="w-full bg-white text-violet-700 hover:bg-white/90 font-bold h-9 rounded-xl border-0 shadow-md transition-all active:scale-95">
                                    Manage Plan
                                </Button>
                            </Link>
                        </div>
                ...
                <div className="bg-white rounded-3xl p-5 shadow-sm border border-neutral-50 relative overflow-hidden group hover:shadow-lg transition-all duration-500">
                     <div className="absolute top-0 right-0 opacity-20 pointer-events-none group-hover:scale-110 transition-transform duration-700">
                        <svg className="w-32 h-32 text-violet-100" viewBox="0 0 200 200" fill="currentColor">
                    ...
                    <div className="grid grid-cols-2 gap-3 relative z-10">
                        {(() => {
                            const actionVariants: any = {
                                violet: {
                                    wrapper: 'bg-violet-50/50 border-violet-100/50 hover:bg-violet-500 hover:shadow-violet-200',
                            ...
                            return [
                                 { label: 'Post Job', icon: Plus, color: 'violet', href: '/jobs' },
                                 { label: 'Candidates', icon: Users, color: 'sky', href: '/candidates' },
                                 { label: 'Messages', icon: MessageCircle, color: 'emerald', href: '/messages' },
                                 { label: 'Settings', icon: Settings, color: 'amber', href: '/settings' },
                            ].map((action, i) => {
                                 const variant = actionVariants[action.color] || actionVariants.violet;
                                 return (
                    ...
                    <div className="space-y-4 relative z-10">
                        {data.recent_activities.slice(0, 4).map((activity: any, i: number) => (
                           <div key={i} className="flex gap-3 p-2 rounded-xl hover:bg-neutral-50 transition-colors cursor-default group/activity">
                                <div className={`w-2 h-2 rounded-full mt-1.5 flex-none ${activity.type === 'job' ? 'bg-violet-500' : 'bg-emerald-500'} group-hover/activity:scale-125 transition-transform`}></div>
                                <div>
                                    <p className="text-xs font-bold text-neutral-800 leading-tight line-clamp-1 group-hover/activity:text-violet-700 transition-colors">{activity.user} {activity.action}</p>
                ...
                <div className="bg-white rounded-3xl p-5 shadow-sm border border-neutral-50 relative overflow-hidden group hover:shadow-lg transition-all duration-500 animate-in fade-in slide-in-from-right-8 duration-700 delay-700 fill-mode-backwards">
                     <div className="flex justify-between items-center mb-4">
                        ...
                        <Button variant="ghost" size="sm" className="h-6 text-[10px] text-neutral-400 hover:text-violet-600 hover:bg-violet-50 rounded-lg px-2">
                            Calendar
                        </Button>
                    </div>
                    ...
                    <div className="space-y-3">
                        {data.upcoming_interviews && data.upcoming_interviews.length > 0 ? (
                            data.upcoming_interviews.map((interview: any, i: number) => (
                                <div key={i} className="flex items-center gap-3 p-3 rounded-2xl bg-neutral-50/50 border border-neutral-100 hover:bg-white hover:border-violet-100 hover:shadow-md transition-all duration-300 group/interview">
                                    {/* Date Badge */}
                                    <div className="flex flex-col items-center justify-center w-12 h-12 rounded-xl bg-white border border-neutral-100 shadow-sm text-center group-hover/interview:border-violet-200 group-hover/interview:bg-violet-50 transition-colors">
                                        <span className="text-[10px] font-bold text-neutral-400 uppercase leading-none group-hover/interview:text-violet-400">{interview.date?.split(' ')[0]}</span>
                                        <span className="text-sm font-black text-neutral-900 leading-none mt-0.5 group-hover/interview:text-violet-700">{interview.date?.split(' ')[1]}</span>
                                    </div>
                                    
                                    <div className="flex-1 min-w-0">
                                        <h4 className="text-xs font-bold text-neutral-900 truncate group-hover/interview:text-violet-700 transition-colors">{interview.candidate_name}</h4>
                                    ...
                                    <Button variant="ghost" size="sm" className="h-8 w-8 rounded-full p-0 text-neutral-400 hover:text-white hover:bg-violet-500 transition-all">
                            ...
                            <div className="text-center py-6">
                                ...
                                <Button variant="ghost" className="text-[10px] text-violet-500 hover:text-violet-600 h-auto p-0 mt-1 hover:bg-transparent underline-offset-4 hover:underline" size="sm">Schedule Now</Button>
                            </div>
                        )}
                    </div>
                </div>
